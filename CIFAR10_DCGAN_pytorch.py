# CIFAR10 image generation using DCGAN
import torch
...
# Plot losses
def plot_loss(d_losses, g_losses, num_epoch, save=False, save_dir='CIFAR10_DCGAN_results/', show=False):
    fig, ax = plt.subplots()
    ...
    if save:
        if not os.path.exists(save_dir):
            os.mkdir(save_dir)
        save_fn = save_dir + 'CIFAR10_DCGAN_losses_epoch_{:d}'.format(num_epoch + 1) + '.png'
        plt.savefig(save_fn)
...
def plot_result(generator, noise, num_epoch, save=False, save_dir='CIFAR10_DCGAN_results/', show=False, fig_size=(5, 5)):
    generator.eval()
    ...
    if save:
        if not os.path.exists(save_dir):
            os.mkdir(save_dir)
        save_fn = save_dir + 'CIFAR10_DCGAN_epoch_{:d}'.format(num_epoch+1) + '.png'
        plt.savefig(save_fn)
...
torch.save(G.state_dict(), "init_G.pth.tar")
torch.save(D.state_dict(), "init_D.pth.tar")
